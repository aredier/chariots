...
class SubVersion(AbstractSubversion):
    ...
    def fields_hash(self) -> Text:
        ...
        for value in self._fields.values():
            res_hash.update(str(str(value)).encode("utf-8"))
    ...
    def update_fields(self, **fields):
        ...
        for linkded_subversion in self.linked_subversions:
            linkded_subversion.update_fields(**{
                str(self._unique_identifier): self.fields_hash
                })
...
class Version:
    ...
    def save_fields(self, path: Text):
        ...
        with open(path, "w") as version_fields_file:
            json.dump({
                ...
                "version_identifiers":{
                    "major": str(self.major._unique_identifier),
                    "minor": str(self.minor._unique_identifier),
                    "patch": str(self.patch._unique_identifier)
                }
            }, version_fields_file)
    ...
    def load_fields(cls, path: Text) -> Mapping[Text, Any]:
        ...
        with open(path, "r") as version_field_file:
            res_json = json.load(version_field_file)
            versioned_identifiers ={
                "major": uuid.UUID(res_json["version_identifiers"]["major"]),
                "minor": uuid.UUID(res_json["version_identifiers"]["minor"]),
                "patch": uuid.UUID(res_json["version_identifiers"]["major"]),
            }
            return res_json["fields"], versioned_identifiers
...
class _VersionField:
    ...
    def __init__(self, default_value: Any = None,
        ...
        self._inner_value = default_value if default_value is not None else default_factory()
