...
def test_app_response(Range10, IsPair, NotOp, tmpdir, opstore_func):  # pylint: disable=invalid-name
    ...
    app =  PipelinesServer([pipe1, pipe], op_store_client=opstore_func(tmpdir), import_name='some_app')
...
def test_app_persistance(enchrined_pipelines_generator, tmpdir, opstore_func):
    ...
    app = PipelinesServer([pipe], op_store_client=op_store_client, import_name='some_app')
    test_client = TestPipelinesClient(app)
