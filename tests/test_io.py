...
@pytest.fixture
def csv_file_no_index(tmp_path):
    data = pd.DataFrame([
        [i, 2 * i, -i]
        for i in range(10)
    ], columns=["x", "y", "z"])
...
def test_csv_tap_no_batch_single_col(csv_file_no_index):
    with csv.CSVTap(csv_file_no_index, {Matrix.with_shape_and_dtype((None, 1), FloatType): ["x"]}) as tap:
        res = UnwrapOp()(tap)
...
def test_csv_tap_no_batch_multiple_cols(csv_file_no_index):
    with csv.CSVTap(csv_file_no_index, {XMarker.with_shape_and_dtype((None, 1), FloatType): ["x"],
...
def test_csv_tap_no_batch_grouped_cols(csv_file_no_index):
    with csv.CSVTap(csv_file_no_index, {XYMarker.with_shape_and_dtype((None, 1), FloatType): ["x", "y"],
...
def test_csv_tap_batched_single_col(csv_file_no_index):
    with csv.CSVTap(csv_file_no_index, {Matrix.with_shape_and_dtype((None, 1), FloatType): ["x"]},
...
def test_csv_tap_batched_multiple_cols(csv_file_no_index):
    with csv.CSVTap(csv_file_no_index, {XMarker.with_shape_and_dtype((None, 1), FloatType): ["x"],
...
def test_csv_tap_batched_grouped_cols(csv_file_no_index):
    with csv.CSVTap(csv_file_no_index, {XYMarker.with_shape_and_dtype((None, 1), FloatType): ["x", "y"],
...
def test_csv_tap_index_single_col(csv_file_index):
    with csv.CSVTap(csv_file_index, {Matrix.with_shape_and_dtype((None, 1), FloatType): ["x"]}) as tap:
        res = UnwrapOp()(tap)
        res_as_a_list = list(res.perform())
        res_as_a_list.should.have.length_of(1)
        res_as_a_list[0].should.be.a(dict)
...
def test_csv_tap_index_multiple_cols(csv_file_index):
    with csv.CSVTap(csv_file_index, {XMarker.with_shape_and_dtype((None, 1), FloatType): ["x"],
                               YMarker.with_shape_and_dtype((None, 1), FloatType): ["y"]}) as tap:
        res = SumOp()(tap)
        res_as_a_list = list(res.perform())
        res_as_a_list.should.have.length_of(1)
...
def test_csv_tap_index_grouped_cols(csv_file_index):
    with csv.CSVTap(csv_file_index, {XYMarker.with_shape_and_dtype((None, 1), FloatType): ["x", "y"],
                               ZMarker.with_shape_and_dtype((None, 1), FloatType): ["z"]}) as tap:
        res = GroupedSumOp()(tap)
        res_as_a_list = list(res.perform())
        res_as_a_list.should.have.length_of(1)
